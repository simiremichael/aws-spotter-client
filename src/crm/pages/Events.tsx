...
import { useGetEventByAgentQuery, useLogoutAgentMutation } from '../../services/api/propertyAPI';
...
@media screen and (max-width: 900px) {
 ...
 `
 const EventDetailsInnerContainer = styled.div`
 margin: 3% 3%;
 min-width: 900px;
 `
 const InnerEventDetails = styled.div`
...
function Events() {
 ...
 const {data} = useGetEventByAgentQuery(agentId);
  ...
  return (
    ...
    <RightContainer>
    <PlusTitleContainer>
    <CreateContactContainer onClick={() => navigate('/calendar')}>
   ...
   <>
    ...
    <NavItemsContainer>
     ...
     <ContactsContainer>
     <ContactsTitle>Events</ContactsTitle>
      <ContactsContent>
      Keep track of all your events and meetings. Create and manage viewings, appointments, and other tasks. Events that include relevant data such as which homes will be viewed and who will be in attend make it easy to take the right decision.
      </ContactsContent>
      <CreateContactsContainer>
      <CreateContacts onClick={() => navigate('/calendar')}>New event</CreateContacts>
      </CreateContactsContainer>
     </ContactsContainer>
   </Grid>
   <Grid item lg={6} md={6} sm={12} xs={12}>
    <IllustrationContainer>
     ...
     : 
     <>
     <Title style={{ marginTop: '80px'}}>Events</Title>
     <EventDetailsContainer>
      <EventDetailsInnerContainer>
       <Grid container style={{border: '0.5px solid gray', margin: 0, padding: 0}}>
      <Grid item lg={2} md={2} sm={2} xs={2} style={{borderRight: '0.5px solid gray', margin: 0, padding: 0}}>
      <DetailsTitle>Title</DetailsTitle>
      </Grid>
      <Grid item lg={6} md={6} sm={6} xs={6} style={{ borderRight: '0.5px solid gray', margin: 0, padding: 0}}>
      <DetailsTitle>Note</DetailsTitle>
      </Grid>
      <Grid item lg={2} md={2} sm={2} xs={2} style={{ borderRight: '0.5px solid gray', margin: 0, padding: 0}}>
      <DetailsTitle>Start</DetailsTitle>
      </Grid>
      <Grid item lg={2} md={2} sm={2} xs={2} style={{ margin: 0, padding: 0}}>
      <DetailsTitle>End</DetailsTitle>
      </Grid>
      </Grid>
      {/* @ts-ignore:next-line */}
      {data?.data?.map((item: any) => 
      <Grid container style={{borderBottom: '0.5px solid gray', borderLeft: '0.5px solid gray', borderRight: '0.5px solid gray'}} key={item._id}>
      <Grid item lg={2} md={2} sm={2} xs={2} style={{borderRight: '0.5px solid gray', margin: 0, padding: 0}}>
      <InnerEventDetails><DetailsText>{item.title}</DetailsText></InnerEventDetails>
      </Grid>
      <Grid item lg={6} md={6} sm={6} xs={6} style={{ borderRight: '0.5px solid gray', margin: 0, padding: 0}}>
      <InnerEventDetails><DetailsText>{item.note}</DetailsText></InnerEventDetails>
      </Grid>
      <Grid item lg={2} md={2} sm={2} xs={2} style={{ borderRight: '0.5px solid gray', margin: 0, padding: 0}}>
      <InnerEventDetails><DetailsText>{item.start}</DetailsText></InnerEventDetails>
      </Grid>
      <Grid item lg={2} md={2} sm={2} xs={2} style={{ margin: 0, padding: 0}}>
      <InnerEventDetails><DetailsText>{item.end}</DetailsText></InnerEventDetails>
      </Grid>
      </Grid>
      )}
      </EventDetailsInnerContainer>
