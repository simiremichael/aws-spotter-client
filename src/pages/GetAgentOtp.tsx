...
import { useGenerateAgentOTPMutation } from '../services/api/propertyAPI';
...
function GetAgentOTP() {

    const [formData, setFormData] = useState({email: ''})

    const navigate = useNavigate()

    const [generateAgentOTP, {isSuccess, isLoading, error}] = useGenerateAgentOTPMutation();

    const handleSubmit = (e: any) => {
        e.preventDefault();
        generateAgentOTP(formData);
    };
console.log(error, formData)

    useEffect(() => { 
    if(isSuccess){
    navigate('/resetAgentPassword')
    }
    }, [isSuccess, navigate]);
