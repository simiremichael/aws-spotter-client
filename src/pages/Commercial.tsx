...
import { selectCurrentCommercial, setCommercial } from '../services/features/commercialSlice';
import {Alert, Grid, Link, Paper, Skeleton, Tooltip } from '@mui/material';
import CommercialPaginate from '../components/CommercialPagination';
...
function Commercial() {
  ...
  const handleSearch = () => {
  // e.preventDefault();
  navigate(`/commercial?search=${search || 'none'}&category=${category}&propertyGroup=${propertyGroup}&type=${type}&bath=${bath}&bed=${bed}&minPrice=${minPrice}&maxPrice=${maxPrice}&sort=${sort}`)
  }
  return (
    <StyledBox>
      <NavBar />
    ...
    { data?.data.length < 1 ? <Alert style={{marginTop: '10px'}} severity="info">No data found!</Alert> : ''}
        ...
        <TopContainer>
          ...
          <SalesContainer>
            {/* @ts-ignore:next-line */}
            <SalesResult>{commercial?.data?.length} results</SalesResult>
        ...
        <SortContainer>
          ...
          <RightContainer>
            <Sort>Sort by:</Sort>
            <FormControl sx={{ marginLeft: '5px', minWidth: 90, }} size="small">
    <InputLabel id="demo-simple-select-autowidth-label">Select</InputLabel>
        ...
        { !commercial?.data && isFetching ?
         ...
         :
          <>
            {/* @ts-ignore:next-line */}
            {commercial?.data.map((result: any) => ( 
              ...
              <CardImg item lg={3} xs={12} md={3} sm={4}>
                ...
                >
                  {/* @ts-ignore:next-line */}
                   { !result.images && isFetching ? <Skeleton variant="rounded" width='100%' height={220} /> : result?.images.map((item: any, index: any) => (
              ...
              <StyledLink href={`/commercialDetailsPage/${result._id}`}>
                  <LeftContainers>
                    ...
                    <DetailContainer>
                      ...
                      <Detail>{!result.address1 && isFetching ? <Skeleton variant="rounded" width='100%' height='40%' /> : result?.address1}</Detail>
                    </DetailContainer>
                  </LeftContainers>
                  </StyledLink>
                  <RightContainers>
                    ...
                    <LogoImg src={!result.loho && isFetching ? <Skeleton variant="circular" width={40} height={40} /> : result.logo} />
                  </RightContainers>
                </CardDetails>
                <BottomContainer>
